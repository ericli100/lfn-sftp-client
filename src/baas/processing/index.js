...
async function getRemoteSftpFiles({ baas, logger, VENDOR_NAME, ENVIRONMENT, config, remoteFileList, correlationId }) {
    ...
    if (output.remoteFileList.length > 0) {
        ...
        for (let file of output.remoteFileList) {
            ...
            catch (processingErrorSFTP) {
                // we will catch the error, report it and let the code move on to the next file.
                await baas.audit.log({ baas, logger, level: 'error', message: `${VENDOR_NAME}: SFTP file processing ERROR [${file.filename}] for environment [${ENVIRONMENT}] with error:[${processingErrorSFTP.message}]`, correlationId })
            }
        } // end the for each file processing
