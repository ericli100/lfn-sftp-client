...
let render_file = function(lines, file_type, date, working_directory, file_name)
{
    return new Promise((resolve, reject) =>
    {
        ...
        if(!file_name) {
            ...
            file_name = `${file_type}_${file_name}`
        };
...
/* MAIN */
async function split_from_json(json_content, date, working_directory, file_name)
{
    ...
    // Render File Content
    let payments_file_content = await renderFileContentFromJSON(split_json_content['payments']);
    let returns_file_content = await renderFileContentFromJSON(split_json_content['returns']);
